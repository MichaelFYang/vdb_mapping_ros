...
template <typename VDBMappingT>
vdb_mapping_msgs::ValueGrid
VDBMappingROS<VDBMappingT>::gridToValue(const typename VDBMappingT::ValueGridT::Ptr update) const
{
  vdb_mapping_msgs::ValueGrid msg;
  msg.map = valueToByteArray(update);
  return msg;
}

template <typename VDBMappingT>
std::string
VDBMappingROS<VDBMappingT>::valueGridToStr(const typename VDBMappingT::ValueGridT::Ptr update) const
{
...
template <typename VDBMappingT>
std::vector<uint8_t>
VDBMappingROS<VDBMappingT>::valueToByteArray(const typename VDBMappingT::ValueGridT::Ptr update) const
{
  std::string map_str = valueGridToStr(update);
  return std::vector<uint8_t>(map_str.begin(), map_str.end());
}
...
template <typename VDBMappingT>
typename VDBMappingT::ValueGridT::Ptr
VDBMappingROS<VDBMappingT>::strToValue(const std::string& msg) const
{
  std::istringstream iss(msg);
  openvdb::io::Stream strm(iss);
  openvdb::GridPtrVecPtr grids;
  ...
  // This cast might fail if different VDB versions are used.
  // Corresponding error messages are generated by VDB directly
  typename VDBMappingT::ValueGridT::Ptr update_grid =
    openvdb::gridPtrCast<typename VDBMappingT::ValueGridT>(grids->front());
  return update_grid;
}
...
template <typename VDBMappingT>
typename VDBMappingT::ValueGridT::Ptr
VDBMappingROS<VDBMappingT>::byteArrayToValueGrid(const std::vector<uint8_t>& msg) const
{
  std::string map_str(msg.begin(), msg.end());
  return strToValue(map_str);
}
